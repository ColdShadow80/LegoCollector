...
passport.deserializeUser((id, done) => {
    db.get("SELECT id, name, email, dark_mode, items_per_page, google_id FROM users WHERE id = ?", [id], (err, row) => {
        ...
        if (!row) return done(null, false); // Proteção contra sessão fantasma
        done(null, row);
    });
});
...
app.get('/dashboard', (req, res) => {
    ...
    const q1 = new Promise(r => db.get(`SELECT COUNT(*) as total_sets, SUM(quantity) as total_items, SUM(sets.num_parts * user_sets.quantity) as total_parts, COALESCE(SUM(user_sets.purchase_price * user_sets.quantity), 0) as total_spent, COALESCE(SUM(sets.price_eur * user_sets.quantity), 0) as total_value_rrp FROM user_sets JOIN sets ON user_sets.set_num = sets.set_num WHERE user_id = ? AND status = 'OWNED'`, [userId], (e, row) => r(row)));
