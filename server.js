...
const fs = require('fs');
const axios = require('axios'); // Necessário para a API do Scanner
const app = express();

const upload = multer({ dest: 'uploads/' });

// ... (Configurações de Email e Passport mantêm-se iguais) ...
const transporter = nodemailer.createTransport({ service: process.env.EMAIL_SERVICE || 'gmail', auth: { user: process.env.EMAIL_USER, pass: process.env.EMAIL_PASS } });
async function sendEmail(to, subject, text) { if (!process.env.EMAIL_USER) { console.log(`\n📨 [EMAIL SIMULADO] Para: ${to}\n${text}\n`); return; } await transporter.sendMail({ from: process.env.EMAIL_USER, to, subject, text }); }
function ensureAdmin(req, res, next) { if (req.user && req.user.id === 1) return next(); res.status(403).send("Acesso Negado."); }
passport.serializeUser((user, done) => done(null, user.id));
passport.deserializeUser((id, done) => { db.get("SELECT id, name, email, dark_mode, items_per_page, google_id FROM users WHERE id = ?", [id], (err, row) => done(err, row)); });
function updateLastLogin(user) { db.run("UPDATE users SET last_login = CURRENT_TIMESTAMP WHERE id = ?", [user.id]); }
passport.use(new LocalStrategy({ usernameField: 'email' }, (email, password, done) => { db.get("SELECT * FROM users WHERE email = ?", [email], async (err, user) => { if (err || !user) return done(null, false, { message: 'Credenciais inválidas.' }); if (!user.password) return done(null, false, { message: 'Use Google Login.' }); if (await bcrypt.compare(password, user.password)) { if (user.is_verified === 0) return done(null, false, { message: 'Conta não ativada.' }); updateLastLogin(user); return done(null, user); } return done(null, false, { message: 'Credenciais inválidas.' }); }); }));
if (process.env.GOOGLE_CLIENT_ID) { passport.use(new GoogleStrategy({ clientID: process.env.GOOGLE_CLIENT_ID, clientSecret: process.env.GOOGLE_CLIENT_SECRET, callbackURL: "/auth/google/callback" }, (accessToken, refreshToken, profile, done) => { const email = profile.emails[0].value; const googleId = profile.id; db.get("SELECT * FROM users WHERE google_id = ? OR email = ?", [googleId, email], (err, user) => { if (user) { if (user.is_verified === 0) db.run("UPDATE users SET is_verified = 1 WHERE id = ?", [user.id]); updateLastLogin(user); return done(null, user); } db.run("INSERT INTO users (name, email, google_id, is_verified, last_login) VALUES (?, ?, ?, 1, CURRENT_TIMESTAMP)", [profile.displayName, email, googleId], function(err) { if(this.lastID === 1) db.run("INSERT OR IGNORE INTO user_sets (user_id, set_num) SELECT 1, set_num FROM sets WHERE owned = 1"); done(null, { id: this.lastID, name: profile.displayName, email }); }); }); })); }

app.set('view engine', 'ejs');
app.use(express.static('public'));
app.use(express.json());
app.use(express.urlencoded({ extended: true }));
app.use(session({ store: new SQLiteStore({ db: 'sessions.db' }), secret: process.env.SESSION_SECRET || 'dev_secret', resave: false, saveUninitialized: false, cookie: { maxAge: 30 * 24 * 3600000 } }));
app.use(passport.initialize());
app.use(passport.session());
app.use((req, res, next) => { res.locals.error = req.query.error; res.locals.success = req.query.success; res.locals.user = req.user; next(); });

// ... (Função buildFilters mantém-se igual) ...
...
// 2. API Scanner (Pesquisa no Rebrickable por EAN/UPC)
app.get('/api/scan', async (req, res) => {
    ...
    try {
        // Pesquisa no Rebrickable
        const url = `https://rebrickable.com/api/v3/lego/sets/?search=${code}&page_size=1`;
        const response = await axios.get(url, { headers: { 'Authorization': `key ${process.env.REBRICKABLE_API_KEY}` }});

        if (response.data.results && response.data.results.length > 0) {
            const set = response.data.results[0];
        ...
        res.status(500).json({ error: 'Erro externo' });
    }
});
